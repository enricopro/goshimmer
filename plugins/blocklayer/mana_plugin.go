	...
	if err := daemon.BackgroundWorker("Mana", func(ctx context.Context) {
		...
		if !readStoredManaVectors() {
			...
			if Parameters.Snapshot.File != "" {
				...
				confirmedConsensusManaByNode := map[identity.ID]float64{}
				...
				utxoStatesConsumer := func(outputsWithMetadatas []*ledger.OutputWithMetadata) {
					...
					processOutputs(outputsWithMetadatas, confirmedConsensusManaByNode, true /* areCreated */)
				...
				epochDiffsConsumer := func(header *ledger.SnapshotHeader, epochDiffs map[epoch.Index]*ledger.EpochDiff) {
					...
					for ei := header.FullEpochIndex + 1; ei <= cManaTargetEpoch; ei++ {
						...
						processOutputs(diff.Created(), confirmedConsensusManaByNode, true /* areCreated */)
						...
						processOutputs(diff.Spent(), confirmedConsensusManaByNode, false /* areCreated */)
...
func readStoredManaVectors() (read bool) {
	for vectorType := range baseManaVectors {
		storages[vectorType].ForEach(func(_ []byte, cachedObject *objectstorage.CachedObject[*mana.PersistableBaseMana]) bool {
...
func storeManaVectors() {
	for vectorType, baseManaVector := range baseManaVectors {
		persistables := baseManaVector.ToPersistables()
...
func pruneStorages() {
	...
	confirmedCManaVectorStorage.Prune()
}
